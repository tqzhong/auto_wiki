...
| `MAX_INPUT_TOKENS` | 目标模型上下文窗口 | 供 proxy 的 `/v1/models` 返回给 Claude Code |
...
**问题**：Claude Code 启动后执行 `/context` 只显示 `200k`，即使第三方后端模型支持 `1M`。原因通常不是后端 API 限制，而是本地 proxy 没有暴露模型发现端点，Claude Code 无法读取 `max_input_tokens`，只能按本地默认/模型别名估算。

**解决**：在 proxy 中实现 Anthropic 风格的 `GET /v1/models` 和 `GET /v1/models/{model}`，返回目标模型的 `max_input_tokens`。同时让 wrapper 默认传真实模型名，例如 `--model mimo-v2.5-pro`，不要只传 `sonnet` 这种 Claude 别名。
